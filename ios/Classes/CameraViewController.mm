...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!_isFoucePixel) {
        _timer = [NSTimer scheduledTimerWithTimeInterval:1.3 target:self selector:@selector(fouceMode) userInfo:nil repeats:true];
    }
    ...
    [_captureOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:
     ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
        if (imageSampleBuffer != NULL) {
            ...
            [self performSelectorOnMainThread:@selector(readyToGetImage:) withObject:resultDict waitUntilDone:false];
            weakSelf.isProcessingImage = false;
        }
    }];
}
       ...
       fromConnection:(AVCaptureConnection *)connection
{
    ...
    //检边识别
    if (_capture == true) { //导航栏动画完成
        if (self.isProcessingImage== false) {  //点击拍照后 不去识别
